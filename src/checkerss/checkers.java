...
public class checkers extends JPanel implements ActionListener, MouseListener {
    ...
    public static int storedRow, storedCol;
    public boolean isJump = false;
    static BufferedImage crownImage = null;
    ArrayList bulundugudurumx = new ArrayList();
    ArrayList bulundugudurumy = new ArrayList();
    ArrayList hamlex = new ArrayList();
    ArrayList hamley = new ArrayList();
    int[][] tahta = new int[10][10];
    static double [][] durumx;
    static double [][] carpim;
    public static String sonhal="";
    public static String sonhal1="";
    public static void main(String[] args) {
        try {
            crownImage = ImageIO.read(new File("C:\\Users\\furkan\\Desktop\\Java-Checkers-master\\crown.png"));
        } catch (IOException e) {
    ...
    public void mousePressed(java.awt.event.MouseEvent evt) {
        ...
        if (inPlay == false && gameData[col][row] != 0&&currentPlayer!=WHITE&&currentPlayer!=WHITE_KING/*|| inPlay == true && checkTeamPiece(col, row) == true*/) {
            resetPlay();
            storedCol = col;
            storedRow = row; // Sets the current click to instance variables to be used elsewhere
            getAvailablePlays(col, row);
        } else if (inPlay == true && availablePlays[col][row] == 1&&currentPlayer!=WHITE&&currentPlayer!=WHITE_KING&&storedCol!=99) {
            makeMove(col, row, storedCol, storedRow);
            getAvailablePlays(col, row);
        } else if (inPlay == true && availablePlays[col][row] == 0&&currentPlayer!=WHITE&&currentPlayer!=WHITE_KING) {
            resetPlay();
        }
    }
    ...
    public void swapPlayer() {
        ...
        if (currentPlayer == RED) {
            ...
            hamlex.clear();
            hamley.clear();
            bulundugudurumx.clear();
            bulundugudurumy.clear();
            durum();
            resetPlay();
        } else {
           storedCol=0;
    ...
    public void durum() {
        for (int i = 0; i < tahta.length; i++) { //tahtayı sıfırlarla doldur.
        ...
        for (int i = 1; i < tahta.length - 1; i++) {
            for (int j = 1; j < tahta.length - 1; j++) {
                tahta[i][j] = gameData[j - 1][i - 1];
        ...
        for (int i = 0; i < gameData.length; i++) {
            for (int j = 0; j < gameData.length; j++) {
                System.out.print(gameData[j][i]);
                sonhal+=gameData[j][i];
                //System.out.print(baseGameData[j][i]);
                //if(checkTeamPiece(i, j))
                oynanyerler(i, j);
        ...
        for (int i = 0; i < hamlex.size(); i++) {
            System.out.println("" + hamlex.get(i) + "---" + hamley.get(i) + "nokta" + bulundugudurumx.get(i) + "++++" + bulundugudurumy.get(i));

        }
         durumx =  new double[hamlex.size()][hamlex.size()];
        Random random=new Random();
        for (int i = 0; i < durumx.length; i++) {
            double toplam=0;
            for (int j = 0; j < durumx.length; j++) {
                durumx[i][j]=random.nextInt(5);
                toplam+=durumx[i][j];
            ...
            double toplam2=0;
            for (int j = 0; j < durumx.length; j++) {
                durumx[i][j]=durumx[i][j]/toplam;
                System.out.print(durumx[i][j]+" ");
                toplam2+= durumx[i][j];
            }
            System.out.println("------------"+ toplam2);
        }
      
        carpim =  new double[hamlex.size()][hamlex.size()];
        for (int i = 0; i < carpim.length; i++) {
            for (int j = 0; j < carpim.length; j++) {
                for (int k = 0; k < carpim.length; k++) {
                 
                    carpim[i][j]+=durumx[i][k]*durumx[k][j];
                }
        ...
        try {
             getAvailablePlays((int)bulundugudurumx.get(x), (int)bulundugudurumy.get(x));
             makeMove((int) hamlex.get(x),(int) hamley.get(x), (int)bulundugudurumx.get(x), (int)bulundugudurumy.get(x));
        } catch (Exception e) {
        }
         ...
         resetPlay();
        //makeMove((int)bulundugudurumx.get(x), (int)bulundugudurumy.get(x),(int) hamlex.get(x),(int) hamley.get(x));
              System.out.println(enbuyuk);   
            repaint(); 
           
    }

    public void oynanyerler(int col, int row) {
        inPlay = true;
        resetPlay();
        getAvailablePlays(col, row);
        if ((checkTeamPiece(col, row) == true)) { //checks if the piece is assigned to the current player
            ...
            if (gameData[col][row] == WHITE_KING) { // Goes up OR down 1 row below it's own
                getUp(col, row);
                getDown(col, row); // GET UP GET UP AND GET DOWN
            }

            for (int j = 0; j < availablePlays.length; j++) {
                for (int i = 0; i < availablePlays.length; i++) {

                    if (availablePlays[j][i] == 1 && gameData[j][i] == 0) {
                        hamlex.add(j);
                        hamley.add(i);
                        bulundugudurumx.add(col);
                        bulundugudurumy.add(row);
                        repaint();
    ...
    public void mousePressedd() {
        ...
        else if (inPlay == true && availablePlays[col][row] == 1&&gameData[storedCol][storedRow]==RED) {
            makeMove(col, row, storedCol, storedRow);
            getAvailablePlays(col, row);
